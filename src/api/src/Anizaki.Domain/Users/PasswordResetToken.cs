...
public sealed class PasswordResetToken : Entity<Guid>
{
        ...
        : base(id)
    {
        ...
        {
            throw new DomainException("Password reset token expiry must be after creation.");
        }
        ...
        DateTime expiresAtUtc)
    {
        ...
        return new PasswordResetToken(id, userId, tokenHash, createdAtUtc, expiresAtUtc, null);
    }
    ...
    public PasswordResetToken MarkUsed(DateTime usedAtUtc)
    {
        var utc = EnsureUtc(usedAtUtc, nameof(usedAtUtc));
        if (!CanBeUsedAt(utc))
        {
            throw new DomainException("Password reset token is not usable.");
        }

        return new PasswordResetToken(Id, UserId, TokenHash, CreatedAtUtc, ExpiresAtUtc, utc);
    }
    ...
    private static Guid EnsureNonEmpty(Guid value, string paramName)
    {
        if (value == Guid.Empty)
        {
            throw new DomainException($"Password reset token '{paramName}' cannot be empty.");
        }
    ...
    private static DateTime EnsureUtc(DateTime value, string paramName)
    {
        ...
        {
            throw new DomainException($"Password reset token '{paramName}' must be UTC.");
        }
